...
class Model(object):
  ...
  def set_time_constants(self):
    ...
    if settings.DECO_MODEL == "ZHL16c":
      ...
      self.tissues[3].set_compartment_time_constants(6.99,    18.5,   10.458, 0.7223, 7.562,  0.7825)
      self.tissues[4].set_compartment_time_constants(10.21,   27.0,   9.220,  0.7582, 6.667,  0.8126)
      self.tissues[5].set_compartment_time_constants(14.48,   38.3,   8.205,  0.7957, 5.933,   0.8434)
      self.tissues[6].set_compartment_time_constants(20.53,   54.3,   7.305,  0.8279, 5.282,  0.8693)
      self.tissues[7].set_compartment_time_constants(29.11,   77.0,   6.502,  0.8553, 4.71,    0.8910)
      self.tissues[8].set_compartment_time_constants(41.20,   109.0,  5.950,  0.8757, 4.187,  0.9092)
      self.tissues[9].set_compartment_time_constants(55.19,   146.0,  5.545,  0.8903, 3.798,  0.9222)
      self.tissues[10].set_compartment_time_constants(70.69,  187.0,  5.333,  0.8997, 3.497,  0.9319)
      self.tissues[11].set_compartment_time_constants(90.34,  239.0,  5.189,  0.9073, 3.223,  0.9403)
      self.tissues[12].set_compartment_time_constants(115.29, 305.0,  5.181,  0.9122, 2.971,  0.9477)
      self.tissues[13].set_compartment_time_constants(147.42, 390.0,  5.176,  0.9171, 2.737,  0.9544)
      self.tissues[14].set_compartment_time_constants(188.24, 498.0,  5.172,  0.9217, 2.523,  0.9602)
      self.tissues[15].set_compartment_time_constants(240.03, 635.0,  5.119,  0.9267, 2.327,  0.9653)
  ...
  def const_depth(self, pressure, seg_time, f_He, f_N2, pp_O2):
    ...
    if pp_O2 > 0.0:
      ...
      if f_He + f_N2 > 0.0:
        p_inert = ambiant_pressure - pp_O2 - tools.calculate_ppH2O_surf(settings.SURFACE_TEMP)
      else:
        p_inert = 0.0
      ...
      if pp_O2_inspired <= ambiant_pressure and p_inert > 0.0:
        ...
        self.ox_tox.add_O2(seg_time, ambiant_pressure - tools.calculate_ppH2O_surf(settings.SURFACE_TEMP))
    else:
      # OC mode
      pp_He_inspired = (ambiant_pressure - tools.calculate_ppH2O_surf(settings.SURFACE_TEMP)) * f_He
      pp_N2_inspired = (ambiant_pressure - tools.calculate_ppH2O_surf(settings.SURFACE_TEMP)) * f_N2
      # update ox_tox
  ...
  def asc_desc(self, start, finish, rate, f_He, f_N2, pp_O2):
    ...
    if pp_O2 > 0.0:
      ...
      p_inert_start = start_ambiant_pressure - pp_O2 - tools.calculate_ppH2O_surf(settings.SURFACE_TEMP)
      p_inert_finish = finish_ambiant_pressure - pp_O2 - tools.calculate_ppH2O_surf(settings.SURFACE_TEMP)
      ...
      pp_He_inspired = (start_ambiant_pressure - tools.calculate_ppH2O_surf(settings.SURFACE_TEMP)) * f_He
      pp_N2_inspired = (start_ambiant_pressure - tools.calculate_ppH2O_surf(settings.SURFACE_TEMP)) * f_N2
      rate_He = rate * f_He
      rate_N2 = rate * f_N2
      # update ox_tox, use average pp_O2
      pp_O2_inspired_avg = ((start_ambiant_pressure - finish_ambiant_pressure) / 2 \
                           + finish_ambiant_pressure - tools.calculate_ppH2O_surf(settings.SURFACE_TEMP)) \
                           * (1.0 - f_He - f_N2)
